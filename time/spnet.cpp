...
{	int i,j,k,jj,dd, exists, r;
	for (i=0;i<Ne;i++) a[i]=0.02/.25;// RS type
	for (i=Ne;i<N;i++) a[i]=0.1/.25; // FS type

	for (i=0;i<Ne;i++) d[i]=8.0*.25; // RS type
	for (i=Ne;i<N;i++) d[i]=2.0*.25; // FS type
...
int main()
{
	...
	initialize();	// assign connections, weights, etc. 
        ...
        int maxtime = 60*25;
	for (sec=0; sec<maxtime; sec++)		// simulation of 1 day
	{
		...
		for (j=0;j<M;j++)
		{
			...
			if (s[i][j]<0) s[i][j]=0.0;
                ...
                if (sec%60==0 || sec==maxtime-1)
                {
			cout << "writing synapses!" << endl;
                        char fna[50], fnb[50];
                        sprintf(fna, "synapses-all-%08d.dat",sec); FILE* fsa = fopen(fna,"w");
                        sprintf(fnb, "synapses-big-%08d.dat",sec); FILE* fsb = fopen(fnb,"w");
                        for (uint ii=0;ii<N;++ii)
 				for (uint ji=0;ji<M;++ji)
					for (uint di=0;di<D;++di)
						for (uint si=0;si<delays_length[ii][di]; ++si)
						{
							if (delays[ii][di][si]!=ji) continue;
							fprintf(fsa, "%d\t%d\t%d\t%f\n", ii, post[ii][ji], di+1, s[ii][ji]);
                					if (s[ii][ji]>=0.95*sm) fprintf(fsb, "%d\t%d\t%d\n", ii, post[ii][ji], di+1, s[ii][ji]);
						}
			fclose(fsa);
			fclose(fsb);
          	}
